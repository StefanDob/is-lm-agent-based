...
public class Haushalt {
	...
	private double reservationsGehalt = 0; 
	...
	private Unternehmen arbeitGeber; 
	
	/** Planned monthly consumption in goods (not money). */
	private double perMonthConsumption; 
	...
	public void dayStep() {
		...
		// If unemployed, check for new jobs
		if(arbeitGeber == null) {
    ...
    public void beginningOfMonth() {
    	...
    	if (!consumptionsFirms.isEmpty() && RandomHelper.nextDouble() < PSI_PRICE) {
    	    ...
    	    //replace firm if it makes sense
    	...
    	//Job search logic
    	if(arbeitGeber == null) {
    		//if unemployed check BETA firms, to find new employment
    		for(int i = 1; i <= BETA; i++) {
    			Unternehmen u = SessionManager.getRandomUnternehmen();
    			if(u.getOpenPosition() && u.getGehalt() >= reservationsGehalt) {
    				acceptPositionAt(u);
    		...
    		// Employed → may randomly search for better jobs
    		...
    		// Dissatisfied → always search at least one firm
    		Unternehmen u = SessionManager.getRandomUnternehmen();
			if(u.getOpenPosition() && u.getGehalt() >= aktuellesGehalt) {
				acceptPositionAt(u);
			}
	...
	/**
     * Selects a firm with probability proportional to the number of workers it employs.
     *
     * @return randomly selected firm, or {@code null} if no firms employ workers
     */
    ...
    /**
     * Selects a firm with probability proportional to demand restrictions faced in the last period.
     *
     * @return selected firm, or {@code null} if no restrictions exist
     */
	...
	/**
     * Receive transfer payment (e.g., social benefits).
     */
